...
class MSP4GROWTHReportGenerator:
    ...
    def _create_data_parameters_page(self) -> List:
        ...
        datasets_data = [["Dataset", "Weight /n Coefficients", "Thresholds /n (Min-Max)", "Type"]]
        ...
        datasets_table = Table(datasets_data, colWidths=[1.4*inch, 1.2*inch, 1.2*inch, 0.8*inch])
    ...
    def _generate_map1(self, result_path: Path, model_name: str) -> Optional[Path]:
        """
    ...
    def _generate_map(self, result_path: Path, model_name: str) -> Optional[Path]:
        """
        Generate a map visualization of the results using Folium.
        ...
        try:
            ...
            # Add a layer from DATA_DIR for context
            Aquaculture_Zones_path = Path( DATA_DIR / "Aquaculture_zones.geojson")  # Adjust path as needed
            if Aquaculture_Zones_path.exists():
                # Load and add Aquaculture Zones layer
                AZ = gpd.read_file(Aquaculture_Zones_path)
                if  AZ.crs.to_epsg() != 4326:
                    AZ = AZ.to_crs(epsg=4326)
                folium.GeoJson(
                    AZ.geometry,
                    name='Aquaculture Zones',
                    style_function=lambda x: {
                        'fillColor': "#21d3ff7d",
                        'weight': 1,
                        'fillOpacity': 0.5
                    },
                    tooltip='Aquaculture Zones'
                ).add_to(m)
            
            # Add aoi polygon if available
            if self.analysis.input_polygon:
                # Create polygon from coordinates
                aoi_polygon = Polygon(self.analysis.input_polygon)
                aoi_gdf = gpd.GeoDataFrame(geometry=[aoi_polygon], crs='EPSG:3857')

                # Create GeoDataFrame for AOI
                if  aoi_gdf.crs.to_epsg() != 4326:
                    aoi_gdf = aoi_gdf.to_crs(epsg=4326)

                # Add AOI polygon to map
                folium.GeoJson(
                    aoi_gdf.geometry,
            ...
            # Color by suitability if available
            # Create a color function based on interest value
            def get_color(interest_value):
                if interest_value is None:
                    return '#gray'
                # Normalize to 0-1 range (assuming 0-4 scale)
                normalized = interest_value / 4.0
                # Use viridis-like color scheme
                if normalized <= 0.25:
            ...
            for idx, (_, row) in enumerate(gdf.iterrows()):
                # Get display FID (starting from 1)
                display_fid = idx + 1
                interest_val = row.get('interest value', 0)
                
                # Create popup content
                popup_html = f"""
                <div style="font-family: Arial, sans-serif; width: 100px;">
                    <h4 style="margin: 0 0 2px 0; color: #2c3e50;">Site {display_fid}</h4>
                    <p style="margin: 5px 0;"><strong>Suitability Score:</strong> {interest_val:.0f}</p>
                    <p style="margin: 5px 0;"><strong>Model:</strong> {self.display_mapper.get_display_name(model_name)}</p>
                </div>
                """
                
                # Create tooltip
                tooltip = f"Site {display_fid}: Score {interest_val:.2f}"
                
                # Add polygon to map
                folium.GeoJson(
                    row.geometry,
                    style_function=lambda feature, color=get_color(interest_val): {
                        'fillColor': color,
                        'color': 'black',
                        'weight': 2,
                        'fillOpacity': 0.7
                ...
                # Add center marker with FID label
                centroid = row.geometry.centroid
                folium.Marker(
            ...
            # Add a custom legend for suitability scores
            legend_html = '''
            <div style="position: fixed; 
                ...
                <h4 style="margin-top: 0; color: #2c3e50;">Suitability Score</h4>
                <div><i style="background:#440154; width:20px; height:15px; float:left; margin-right:8px; border: 1px solid black;"></i>0.0 - 1.0</div>
                <div><i style="background:#31688e; width:20px; height:15px; float:left; margin-right:8px; border: 1px solid black;"></i>1.0 - 2.0</div>
                <div><i style="background:#35b779; width:20px; height:15px; float:left; margin-right:8px; border: 1px solid black;"></i>2.0 - 3.0</div>
                <div><i style="background:#fde725; width:20px; height:15px; float:left; margin-right:8px; border: 1px solid black;"></i>3.0 - 4.0</div>
            </div>
            '''
            ...
            <div style="position: fixed; 
                        ...
                        padding: 10px; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.3);
                        color: #2c3e50;">
                {self.display_mapper.get_display_name(model_name)} Model Results
            </div>
            '''
            m.get_root().html.add_child(folium.Element(title_html))
            
            # Save map
            map_filename = f"map_{model_name}_{self.analysis.request_id}.html"
            map_path = self.analysis.results_dir / map_filename
            m.save(str(map_path))
            
            logger.info(f"Successfully generated Folium map: {map_path}")
            return None  # Return None since we can't embed HTML in PDF
